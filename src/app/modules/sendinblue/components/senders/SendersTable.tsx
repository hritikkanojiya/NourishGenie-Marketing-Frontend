...
import {
  GetSenderOptions,
  SendInBlueSender,
} from "../../models/sendinblue_sender.model";
import {
  CURRENT_PAGE,
  getMetaDataValues,
  getUrlParamString,
...
import { SIBSenderFilterDropdown } from "./SIBSenderFilterDropdown";
...
type Props = {
  openModal: (
    ...
    senderDetails: SendInBlueSender | {}
  ) => any;
  reRenderComponent: boolean;
};

export const SendInBlueSendersTable: FC<Props> = ({
  openModal,
  reRenderComponent,
}) => {
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.id === "selectAll") {
      ...
      setCheckedSenders(
        checked
          ? state.senders?.map((sender) => sender.appSendInBlueSenderId || "")
          : []
      );
      ...
      setCheckedSenders((prevcheckedSenders) =>
        checked
          ...
          : prevcheckedSenders.filter((senderId) => senderId !== id)
      );
  ...
  const onClickDelete = () => {
    handleAlert(
      ...
      "Confirm",
      async () => {
        const request = await SendInBlueSenderService.deleteSender(
          checkedSenders
        );
        if ("data" in request) {
          getSenders({
  ...
  const deleteSender = (sender: SendInBlueSender) => {
    handleAlert(
      `Are you sure?`,
  ...
  const synchronizeSenders = async () => {
    const request = await SendInBlueSenderService.synchronizeSenders();
    if ("data" in request) {
      showToast(request.data.message, "success");
    }
  };

  const getSenders = async (options?: GetSenderOptions) => {
    try {
      setLoading(true);
      const request = await SendInBlueSenderService.getSenders(options);
      ...
      if ("data" in response && "message" in response.data) {
        ...
        setState((previousState) => {
          return {
            ...previousState,
            senders: response.data.sendInBlueSenders,
  ...
  useEffect(() => {
    ...
    const filter: GetSenderOptions = {
      isVerified: Boolean(params.get("isVerified")) || null,
      sendInBlueSenderId: Number(params.get("sendInBlueSenderId")) || null,
  ...
  useEffect(() => {
    if (reRenderComponent) {
      ...
      let filter: GetSenderOptions = {
        appSendInBlueSenderId: filterOptions.appSendInBlueSenderId,
        search: filterOptions.search,
        isVerified: filterOptions.isVerified,
        sendInBlueSenderId: filterOptions.sendInBlueSenderId,
      };
    ...
    sortState.sortOn,
    sortState.sortBy,
    filterOptions.appSendInBlueSenderId,
    filterOptions.isVerified,
    filterOptions.search,
    filterOptions.sendInBlueSenderId,
    filterOptions.page,
    reRenderComponent,
  ]);
          ...
          <div className="card-title">
            <select
              ...
              onChange={(e: any) => {
                ...
                getSenders({ metaData: { limit: e.target.value } });
              }}
            >
          ...
          >
            ...
            <tbody className="fw-semibold text-gray-600">
              {state.senders?.map((sender) => {
                return (
                  <tr key={sender.appSendInBlueSenderId}>
                    <td>
                      <div className="form-check form-check-sm form-check-custom form-check-solid">
                        <input
                          id={sender.appSendInBlueSenderId}
                    ...
                    <td className="mw-200px text-truncate">
                    ...
                    <td className="mw-75px text-center">
                      <span
                        className={clsx("badge badge-sm badge-inline px-2", {
                          "badge-light-success": sender.isVerified,
                          "badge-light-danger": !sender.isVerified,
                        })}
                      >
                        {sender.isVerified ? "Yes" : "No"}
                      </span>
                    </td>
