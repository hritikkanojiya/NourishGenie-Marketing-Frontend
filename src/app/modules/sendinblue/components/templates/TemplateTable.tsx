...
import { SIBTemplateFilterDropdown } from "./SIBTemplateFilterDropdown";
import { TestSIBTemplate } from "./TestSIBTemplateModal";
...
export const SendInBlueTemplatesTable: FC<Props> = ({
  openModal,
  reRenderComponent,
}) => {
  ...
  const openTestModal = (templateId: string) =>
    setModalState({ showModal: true, templateId: templateId });
  ...
  const [filterOptions, setFilterOptions] = useState<GetTemplateOptions>({
    ...
    appSendInBlueTemplateId: null,
    isTestSent: null,
    search: null,
    sendInBlueTemplateId: null,
    page: 1,
  });
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.id === "selectAll") {
      ...
      setCheckedTemplates(
        checked
          ? state.templates?.map(
              (template) => template.appSendInBlueTemplateId || ""
            )
          : []
      );
  ...
  const onClickDelete = () => {
    handleAlert(
      ...
      "Confirm",
      async () => {
        const request = await SendInBlueTemplateService.deleteTemplates(
          checkedTemplates
        );
        if ("data" in request) {
          getTemplates({
  ...
  const deleteTemplates = (template: SendInBlueTemplate) => {
    handleAlert(
      `Are you sure?`,
      ...
      async () => {
        const request = await SendInBlueTemplateService.deleteTemplates([
          template.appSendInBlueTemplateId || "",
        ]);
        if ("data" in request) {
          getTemplates({
            ...filterOptions,
  ...
  const getTemplates = async (options?: GetTemplateOptions) => {
    try {
      setLoading(true);
      const request = await SendInBlueTemplateService.getTemplates(options);
      ...
      if ("data" in response && "message" in response.data) {
        ...
        setState((previousState) => {
          return {
            ...previousState,
            templates: response.data.sendInBlueTemplates,
            templateCount: totalRecords,
          };
        });
  ...
  useEffect(() => {
    ...
    const filter: GetTemplateOptions = {
      appSendInBlueSenderId:
        Number(params.get("appSendInBlueSenderId")) || null,
      sendInBlueTemplateId: Number(params.get("sendInBlueTemplateId")) || null,
  ...
  useEffect(() => {
    if (reRenderComponent) {
      ...
      if (state.templateCount > 0 && offset > state.templateCount) {
        const lastPage = Math.ceil(
          state.templateCount / paginationState.itemsPerPage
        );
      ...
      let filter: GetTemplateOptions = {
        ...
        appSendInBlueTemplateId: filterOptions.appSendInBlueTemplateId,
        isTestSent: filterOptions.isTestSent,
        sendInBlueTemplateId: filterOptions.sendInBlueTemplateId,
      };
    ...
    sortState.sortOn,
    sortState.sortBy,
    filterOptions.appSendInBlueTemplateId,
    filterOptions.isTestSent,
    filterOptions.appSendInBlueSenderId,
    filterOptions.sendInBlueTemplateId,
          ...
          <div className="card-title">
            <select
              ...
              onChange={(e: any) => {
                ...
                getTemplates({ metaData: { limit: e.target.value } });
              }}
            >
        ...
        <div className="card-body pt-0">
          ...
          >
            ...
            <tbody className="fw-semibold text-gray-600">
              {state.templates?.map((template) => {
                return (
                  <tr key={template.appSendInBlueTemplateId}>
                    <td>
                      <div className="form-check form-check-sm form-check-custom form-check-solid">
                        <input
                          id={template.appSendInBlueTemplateId}
                    ...
                    <td className="cursor-pointer">
                      <CopyToClipboard
                        ...
                        text={template.appSendInBlueTemplateId || ""}
                      >
                        <span className="badge badge-light-dark text-hover-primary text-gray-700 mb-1">
                          {`${template.appSendInBlueTemplateId?.substring(
                            0,
                            10
                          )}...`}
                        </span>
                    ...
                    <td className="mw-75px text-center">
                      <span
                        className={clsx(
                          "badge badge-sm badge-light badge-inline px-2",
                          {
                      ...
                      >
                        ...
                        <div className="menu-item px-3">
                          <a
                            ...
                            onClick={() => deleteTemplates(template)}
                          >
                            Delete
                          </a>
                        </div>
          ...
          <div className="d-flex justify-content-between mt-5">
            <PaginationComponent
              ...
              totalRecords={state.templateCount}
