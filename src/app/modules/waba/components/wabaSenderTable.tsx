...
import { GetWABASendersOptions, WABASender } from "../models/waba_sender.model";
import { WABASenderService } from "../services/waba_sender.service";
import { SenderFilterDropdown } from "./WABASenderFilterDropdown";
import { MetaData } from "../../../common/globals/common.model";

type Props = {
  openModal: (
    mode: "create" | "edit" | "view",
    WABASenderDetails: WABASender | {}
  ) => any;
  reRenderComponent: boolean;
};

export const WABASendersTable: FC<Props> = ({
  openModal,
  reRenderComponent,
}) => {
  ...
  const [filterOptions, setFilterOptions] = useState<GetWABASendersOptions>({
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.id === "selectAll") {
      ...
      setCheckedSenders(
        checked
          ? state.WABASenders?.map((constant) => constant.appWABASenderId || "")
          : []
      );
      ...
      setCheckedSenders((prevcheckedSenders) =>
        checked
          ...
          : prevcheckedSenders.filter((constantId) => constantId !== id)
      );
  ...
  const onClickDelete = () => {
    handleAlert(
      ...
      "Confirm",
      async () => {
        const request = await WABASenderService.deleteSenders(checkedSenders);
        if ("data" in request) {
          getWABASenders({
            ...filterOptions,
            metaData: {
  ...
  const deleteWABASenders = (sender: WABASender) => {
    handleAlert(
      `Are you sure?`,
      `Delete this sender <br> ${sender.name}`,
      "question",
      "Confirm",
      async () => {
        const request = await WABASenderService.deleteSenders([
          sender.appWABASenderId || "",
        ]);
        if ("data" in request) {
          getWABASenders({
            ...filterOptions,
  ...
  const getWABASenders = async (options?: GetWABASendersOptions) => {
    try {
      setLoading(true);
      const request = await WABASenderService.getSenders(options);
      ...
      if ("data" in response && "message" in response.data) {
        ...
        setState((previousState) => {
          return {
            ...previousState,
            WABASenders: response.data.WABASenders,
            sendersCount: totalRecords,
          };
        });
  ...
  useEffect(() => {
    ...
    const filter: GetWABASendersOptions = {
  ...
  useEffect(() => {
    if (reRenderComponent) {
      ...
      if (state.sendersCount > 0 && offset > state.sendersCount) {
        const lastPage = Math.ceil(
          state.sendersCount / paginationState.itemsPerPage
        );
      ...
      let filter: GetWABASendersOptions = {
          ...
          <div className="card-title">
            <select
              ...
              onChange={(e: any) => {
                ...
                getWABASenders({ metaData: { limit: e.target.value } });
              }}
            >
            ...
            >
              {checkedSenders?.length > 0 ? (
                ...
                <>
                  ...
                  <SenderFilterDropdown setFilterOptions={setFilterOptions} />
                </>
              )}
        ...
        <div className="card-body pt-0">
          ...
          >
            <thead>
              <tr className="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                ...
                <th id="description" className={clsx("min-w-125px")}>
                  Description
                </th>
                <th
                  id="provider"
            ...
            <tbody className="fw-semibold text-gray-600">
              {state.WABASenders?.map((sender) => {
                return (
                  <tr key={sender.appWABASenderId}>
                    <td>
                      <div className="form-check form-check-sm form-check-custom form-check-solid">
                        <input
                          id={sender.appWABASenderId}
                    ...
                    <td className="cursor-pointer">
                      <CopyToClipboard
                        ...
                        text={sender.appWABASenderId || ""}
                      >
                        <span className="badge badge-light-dark text-hover-primary mb-1">
                          {`${sender.appWABASenderId?.substring(0, 10)}...`}
                        </span>
                      </CopyToClipboard>
                    </td>
                    <td className="mw-125px text-truncate">
                      <span className="text-gray-800 mb-1">{sender.name}</span>
                    </td>
                    <td className="mw-125px text-truncate">
                    ...
                    <td className="mw-75px text-center">
                      <span
                        className={clsx(
                          "badge badge-sm mb-1",
                          {
                            "badge-light-primary": sender.provider === "D360",
                          },
                          {
                            "badge-light-info":
                              sender.provider === "ValueFirst",
                          }
                        )}
                      >
                        {sender.provider}
                    ...
                    <td className="mw-150px fw-bold text-center">
                      <span>
                        {sender?.createdAt
                          ? format(new Date(sender?.createdAt), IST_DATE_FORMAT)
                          : ("" as ReactNode)}
                      </span>
                    </td>
                      ...
                      >
                        ...
                        <div className="menu-item px-3">
                          <a
                            ...
                            onClick={() => deleteWABASenders(sender)}
                          >
                            Delete
                          </a>
                        </div>
          ...
          <div className="d-flex justify-content-between mt-5">
            <PaginationComponent
              ...
              totalRecords={state.sendersCount}
