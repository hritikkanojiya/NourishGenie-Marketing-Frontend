...
import {
  ...
  getUrlParamString,
  onChangeSortObj,
  parseParamValueFromObjectString,
  sortObj,
...
import { AppList, GetAppListOptions } from "../models/app_list.model";
...
type Props = {
  openModal: (
    ...
    appConstantDetails: AppList | {}
  ) => any;
  reRenderComponent: boolean;
};

export const AppListsTable: FC<Props> = ({ openModal, reRenderComponent }) => {
  ...
  const [filterOptions, setFilterOptions] = useState<GetAppListOptions>({
    appListId: null,
    name: null,
    sendInBlue: {
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.id === "selectAll") {
      ...
      setCheckedLists(
        checked
          ? state.appLists?.map((constant) => constant.appListId || "")
          : []
      );
      ...
      setCheckedLists((prevcheckedLists) =>
        checked
          ? [...prevcheckedLists, id]
          : prevcheckedLists.filter((constantId) => constantId !== id)
      );
  ...
  const onClickDelete = () => {
    handleAlert(
      ...
      `You won't be able to revert this! </br> Total ${
        checkedLists.length
      } record${checkedLists.length > 1 ? "s" : ""} will be deleted.`,
      "question",
      "Confirm",
      async () => {
        const request = await AppListService.deleteAppLists(checkedLists);
        if ("data" in request) {
          getAppLists({
            ...filterOptions,
            metaData: {
  ...
  const deleteAppLists = (constant: AppList) => {
    handleAlert(
      `Are you sure?`,
      ...
      "Confirm",
      async () => {
        const request = await AppListService.deleteAppLists([
          constant.appListId || "",
        ]);
        if ("data" in request) {
          getAppLists({
  ...
  const getAppLists = async (options?: GetAppListOptions) => {
    try {
      setLoading(true);
      const request = await AppListService.getAppLists(options);
      ...
      if ("data" in response && "message" in response.data) {
        ...
        setState((previousState) => {
          return {
            ...previousState,
            appLists: response.data.appLists,
            listsCount: totalRecords,
          };
        });
  ...
  useEffect(() => {
    ...
    const filter: GetAppListOptions = {
      name: params.get("name") || null,
      sendInBlue: {
        sendInBlueFolderId: parseParamValueFromObjectString(
          location.search,
          "sendInBlue",
          "sendInBlueFolderId"
        ),
        sendInBlueListId: parseParamValueFromObjectString(
          location.search,
          "sendInBlue",
          "sendInBlueListId"
        ),
  ...
  useEffect(() => {
    if (reRenderComponent) {
      ...
      if (state.listsCount > 0 && offset > state.listsCount) {
        const lastPage = Math.ceil(
          state.listsCount / paginationState.itemsPerPage
        );
      ...
      let filter: GetAppListOptions = {
        appListId: filterOptions.appListId,
        search: filterOptions.search,
        name: filterOptions.name,
        sendInBlue: filterOptions.sendInBlue,
      };
    ...
    sortState.sortOn,
    sortState.sortBy,
    filterOptions.appListId,
    filterOptions.name,
    filterOptions.sendInBlue?.sendInBlueFolderId,
    filterOptions.sendInBlue?.sendInBlueListId,
          ...
          <div className="card-title">
            <select
              ...
              onChange={(e: any) => {
                ...
                getAppLists({ metaData: { limit: e.target.value } });
              }}
            >
            ...
            >
              {checkedLists?.length > 0 ? (
                ...
                <>
                  ...
                  <ListFilterDropdown setFilterOptions={setFilterOptions} />
                </>
              )}
        ...
        <div className="card-body pt-0">
          ...
          >
            <thead>
              <tr className="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                ...
                <th
                  ...
                  onClick={onClickTableHeader}
                  className={clsx(
                    "min-w-200px table-sort cursor-pointer",
                    {
                      "table-sort-asc":
                ...
                <th
                  ...
                  className={clsx(
                    ...
                    {
                      "table-sort-asc":
                        sortState.sortOn === "totalContacts" &&
                        sortState.sortBy === "asc",
                    },
                    {
                      "table-sort-desc":
            ...
            <tbody className="fw-semibold text-gray-600">
              {state.appLists?.map((lists) => {
                return (
                  <tr key={lists.appListId}>
                    <td>
                      <div className="form-check form-check-sm form-check-custom form-check-solid">
                        <input
                          ...
                          type="checkbox"
                          checked={checkedLists.includes(lists.appListId || "")}
                          onChange={handleCheckboxChange}
                        />
                      </div>
                    </td>
                    ...
                    <td className="mw-150px text-center">
                      <span
                        className={clsx(
                          "badge badge-sm badge-square badge-light-success"
                        )}
                      >
                        {lists.totalContacts}
                      </span>
                    ...
                    <td className="mw-150px fw-bold text-center">
                      <span>
                        {lists?.createdAt
                          ? format(new Date(lists?.createdAt), IST_DATE_FORMAT)
                          : ("" as ReactNode)}
                      </span>
                    </td>
