  ...
  return (
    ...
    >
      ...
      {config.app?.pageTitle?.display && pageTitle && (
        <h1
          className={clsx(
            "mb-1 page-heading d-flex text-dark fw-bold fs-3 my-0",
            {
            ...
            <ul className="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
              {Array.from(pageBreadcrumbs).map((item, index) => (
                <React.Fragment key={index}>
                  ...
                  <i className="fs-7 px-1 fa-solid fa-angles-right"></i>
                </React.Fragment>
              ))}
              <li className="ps-0 breadcrumb-item text-dark">{pageTitle}</li>
            </ul>
          </>
        )}
    </div>
